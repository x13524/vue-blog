...
router.get('/api/articles', async (req, res) => {
    ...
    const limit = req.query.payload.limit - 0 || 10
    const skip = limit * (page - 1) // 忽略之前的数据条数
    const ArticleList = (value && value !== '全部')
        ? db.Article.find({tags: value, isPublish: true})
        : db.Article.find({isPublish: true})
    res.send(ArticleList.sort({date: -1}).limit(limit).skip(skip).exec())

    // const query = (value && value !== '全部') ? {tags: value, isPublish: true} : {isPublish: true}
    // const result = await Promise.all([
    //     db.Article.find(query).count(),
    //     db.Article.find(query).sort({date: -1}).limit(limit).skip(skip).exec()
    // ])
    // res.send({
    //     total: result[0],
    //     articles: result[1]
